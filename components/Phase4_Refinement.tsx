...
const Phase4Refinement: React.FC<Phase4Props> = ({ agenda, initialCards, onGenerateNewCards }) => {
    ...
    const handleDownloadPdf = async () => {
        ...
        const selectedCards = cards.filter(card => votes[card.id]);
        await generatePdf(agenda, selectedCards);
            ...
            <div className="bg-white dark:bg-gray-900/50 p-6 rounded-xl shadow-lg mt-12 sticky bottom-4 z-10 border dark:border-gray-700">
                <div className="flex justify-between items-center">
                    ...
                    <button 
                        ...
                        disabled={selectedCards.length === 0 || isDownloading}
                    ...
                    >
                       {isDownloading ? <LoadingSpinner /> : <PdfIcon />}
                       {isDownloading ? 'Generating...' : 'Download Plan as PDF'}
                    </button>
                </div>
            </div>
        </div>
